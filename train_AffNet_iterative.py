...
parser.add_argument('--baum-iters', type=int, default=1, metavar='N',
                    help='how many internal patch transform iterations will be done')
...
suffix= args.expname + '_iterative_6Brown_' + str(args.baum_iters) + '_'  + args.merge + '_' + args.descriptor + '_' + str(args.lr) + '_' + str(args.n_pairs) 
##########################################3
def create_loaders():
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, data in pbar:
        ...
        ##Iterations 
        if epoch > 0:
            for it in range(args.baum_iters):
                out_a_aff_curr, out_p_aff_curr = model(out_patches_a_crop,True), model(out_patches_p_crop,True)
                out_p_aff = torch.bmm(out_p_aff,out_p_aff_curr)
                out_a_aff = torch.bmm(out_a_aff, out_a_aff_curr)
                ######Apply rot and get sifts
                out_patches_a_crop = extract_and_crop_patches_by_predicted_transform(data_a_aff, out_a_aff, crop_size = model.PS)
                out_patches_p_crop = extract_and_crop_patches_by_predicted_transform(data_p_aff, out_p_aff, crop_size = model.PS)
                desc_a,desc_p = descriptor(out_patches_a_crop), descriptor(out_patches_p_crop)
                descr_loss += loss_HardNet(desc_a,desc_p, anchor_swap = True); 
        loss = descr_loss
        #if args.merge == 'sum':
        #    loss = descr_loss
