...
parser.add_argument('--hn', default=1.0, type=float,
                    help='HardNet loss coeff, default 1.0')

parser.add_argument('--mlpred', default=0.0, type=float,
                    help='Matchability prediction loss coeff, default 0')

parser.add_argument('--reppred', default=0.0, type=float,
                    help='Repeatability prediction loss coeff, default 0')
parser.add_argument('--rep', default=0.0, type=float,
                    help='Repeatability loss coeff, default 0')
parser.add_argument('--uniqpred', default=0.0, type=float,
                    help='Uniqueness prediction loss coeff, default 0')
#parser.add_argument('--rep', default=0.0, type=float,
#                    help='Repeatability loss coeff, default 0')
...
suffix='ONet_' + args.merge + '_' + args.descriptor + '_' + str(args.lr) + '_' + str(args.n_pairs) +'_HN' +str(args.hn)  + '_RepPred' + str(args.reppred) + '_MLPred' + str(args.mlpred) + '_Rep'  + str( args.rep)
##########################################3
def create_loaders():
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, data in pbar:
        ...
        out_a_rot, a_rot_u, a_rot_r, a_rot_m = model(data_a_rot_crop,MODE)
        out_p, p_u, p_r, p_m = model(data_p_crop,MODE)
        out_a, a_u, a_r, a_m = model(data_a[:,:, st:fin, st:fin].contiguous(), MODE)
        ...
        loss_hn = loss_HardNet(desc_a,desc_p, return_mean = False)
        ...
        ## Additional losses
        # Matchability 
        sorted_hn_loss, indices = torch.sort(loss_hn)
        m_loss = torch.clamp(0.5 - (sorted_hn_loss[indices[0:-1]].detach() - sorted_hn_loss[indices[1:]].detach()) * (a_m[indices[0:-1]] - a_m[indices[1:]]), min = 0, max = 1.0).mean()
        # Repeatability
        out_patches_a_crop_orig = extract_and_crop_patches_by_predicted_transform(data_a, out_a, crop_size = model.PS)
        desc_a_orig = descriptor(out_patches_a_orig)
        repeatab_descr_dist =  torch.sqrt(((desc_a - desc_a_orig)**2).view(data_a.size(0),-1).sum(dim=1) + 1e-6) # float(desc_a.size(1))
        sorted_dist, ind2 = torch.sort(loss_hn)
        r_loss = torch.clamp(0.5 - (sorted_dist[ind2[0:-1]].detach() - sorted_dist[ind2[1:]].detach()) * (a_r[ind2[0:-1]] - a_r[ind2[1:]]), min = 0, max = 1.0).mean()
        # Uniqueness
        #Done
        loss = args.hn * loss_hn.mean() + args.reppred * r_loss + args.mlpred * m_loss + repeatab_descr_dist.mean() * args.rep
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if batch_idx % 10 == 0:
            pbar.set_description(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, {},{},{}'.format(
                    ...
                    loss.data[0], geom_dist.mean().data[0], r_loss.data[0], m_loss.data[0]))
...
def test(test_loader, model, epoch):
    ...
    geom_distances, desc_distances,m_losses,r_losses = [], [],[],[]
    ...
    for batch_idx, (data_a, data_p) in pbar:
        ...
        r_losses.append(r_loss.data.cpu().numpy().reshape(-1,1))
        m_losses.append(m_loss.data.cpu().numpy().reshape(-1,1))
    ...
    m_losses = np.vstack(m_losses).reshape(-1,1)
    r_losses = np.vstack(r_losses).reshape(-1,1)
    print('\33[91mTest set: Geom MSE: {:.6f}\n\33[0m'.format(geom_distances.mean()))
    print('\33[91mTest set: Desc dist: {:.6f}\n\33[0m'.format(desc_distances.mean()))
    print('\33[91mTest set: Matchability prediction loss: {:.6f}\n\33[0m'.format(m_losses.mean()))
    print('\33[91mTest set: Repeatability prediction loss: {:.6f}\n\33[0m'.format(r_losses.mean()))
    return
